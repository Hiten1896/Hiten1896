...
def generate_year_svg(data):
    ...
    for idx, day in enumerate(data["days"][:364]):
        x = grid_x + (idx // 7) * 14
        y = grid_y + (idx % 7) * 14
        color = "#39d353" if day["count"] > 5 else ("#26a641" if day["count"] > 0 else "#161b22")
        rects.append(f'<rect x="{x}" y="{y}" width="11" height="11" fill="{color}" rx="2"/>')
    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}" width="{w}" height="{h}">
  <style>.bg {{ fill: #0d1117; rx: 12px; }} .border {{ fill: none; stroke: #30363d; stroke-width: 1.5; rx: 12px; }}</style>
  <rect class="bg" width="{w}" height="{h}" /><rect class="border" width="{w-2}" height="{h-2}" x="1" y="1" />
  <text x="24" y="25" font-size="14" font-weight="600" fill="#c9d1d9">{data['total_conts']} contributions in the last 364 days</text>
  {"".join(rects)}
</svg>"""
